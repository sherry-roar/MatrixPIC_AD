...
WarpX::InitTempVectors()
{
    ...
    m_thread_private_sxm.resize(max_threads);
    m_thread_private_sym.resize(max_threads);
    m_thread_private_szm.resize(max_threads);
    m_thread_private_newbin.resize(max_threads);
    ...
    for (int i = 0; i < max_threads; ++i) {
        ...
        m_thread_private_wqz[i].reserve(np);
        m_thread_private_newbin[i].reserve(np);
        
        m_thread_private_sxm[i].reserve(np*(WarpX::nox+1));
        m_thread_private_sym[i].reserve(np*(WarpX::nox+1));
        m_thread_private_szm[i].reserve(np*(WarpX::nox+1));

        m_thread_private_xrhocells[i].reserve(len_rhocell);
        m_thread_private_yrhocells[i].reserve(len_rhocell);
        m_thread_private_zrhocells[i].reserve(len_rhocell);
    }
}
...
WarpX::ReadParameters ()
{
...

    {
        ...
        // printf("parser1 1!!\n");
        // --- Read Rank-Local Sort Parameters ---
        utils::parser::queryWithParser(pp_warpx, "sort_interval", m_sort_interval);
        utils::parser::queryWithParser(pp_warpx, "min_sort_interval", m_min_sort_interval);
        utils::parser::queryWithParser(pp_warpx, "sort_trigger_rebuild_count", m_sort_trigger_rebuild_count);
        utils::parser::queryWithParser(pp_warpx, "sort_trigger_empty_ratio", m_sort_trigger_empty_ratio);
        utils::parser::queryWithParser(pp_warpx, "sort_trigger_full_ratio", m_sort_trigger_full_ratio); // 如果需要再取消注释
        utils::parser::queryWithParser(pp_warpx, "sort_trigger_perf_enable", m_sort_trigger_perf_enable);
        utils::parser::queryWithParser(pp_warpx, "sort_trigger_perf_degrad", m_sort_trigger_perf_degrad);
        utils::parser::queryWithParser(pp_warpx, "gap_ratio", m_gap_ratio);
        utils::parser::queryWithParser(pp_warpx, "rebuild_weight", m_rebuild_weight);
        utils::parser::queryWithParser(pp_warpx, "m_fast_cmp_block_size", m_fast_cmp_block_size);
        utils::parser::queryWithParser(pp_warpx, "m_max_moved_np_rate", m_max_moved_np_rate);
        utils::parser::queryWithParser(pp_warpx, "m_init_np", m_init_np);
        utils::parser::queryWithParser(pp_warpx, "m_len_rhocell", m_len_rhocell);
        // printf("parser!!\n");
        if (m_sort_interval<0) { // 只有当用户指定时才解析
