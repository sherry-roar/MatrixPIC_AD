 ...
 */
 ...
 #pragma omp single
     ...
     local_jx_test.resize(num_threads);
 ...
 void
 WarpXParticleContainer::SortParticlesByCell (amrex::IntVect bin_size=IntVect(AMREX_D_DECL(1, 1, 1)))
 {
     BL_PROFILE("WarpXParticleContainer::SortParticlesByBin()");
    //  printf("Doing global SORT!!!\n");
     // printf("WarpXParticleContainer::SortParticlesByCell() is called\n");
     float gap_ratio=WarpX::GetInstance().m_gap_ratio;
     // printf("gap %f\n",gap_ratio);
     // amrex::Abort("test");
     // 如果 bin_size 是 0 向量，不执行
     if (bin_size == amrex::IntVect::TheZeroVector()) { return; }
 
     // 遍历所有 refinement levels
     for (int lev = 0; lev < numLevels(); ++lev)
     {
         ...
         // 准备一个 DenseBins (或你若已在类里定义了 m_bins ，也可复用)
         // 这里用局部 bins，就和 ParticleContainer_impl 里的用法一致
         // amrex::DenseBins<ParticleTileDataType> bins;
 
         // 用 MakeMFIter(lev) 遍历该 level 下的所有 tile
         ...
         for (WarpXParIter pti(*this, lev); pti.isValid(); ++pti)
         {
             ...
             // ptile.idd=idd;
             // amrex::Print() << ptile.idd <<" A cell-centered Box " << box << "\n";
             idd++;
 
 
             // 计算在 box 上分成多少 bin
             int ntiles = numTilesInBox(box, /*tiling=*/true, bin_size);
             ...
             std::vector<int> perm1(np);
             std::iota(perm1.begin(), perm1.end(), 0); // iota 函数将 perm 数组填充为 0, 1, 2, ..., np-1   
 ...
 #if 0 
 void
 WarpXParticleContainer::SortParticlesByCell_0522 (amrex::IntVect bin_size=IntVect(AMREX_D_DECL(1, 1, 1)))
 {
     BL_PROFILE("WarpXParticleContainer::SortParticlesByBin()");
     // printf("WarpXParticleContainer::SortParticlesByCell() is called\n");
     float gap_ratio=WarpX::GetInstance().m_gap_ratio;
     ...
     for (int lev = 0; lev < numLevels(); ++lev)
     {
         ...
         for (WarpXParIter pti(*this, lev); pti.isValid(); ++pti)
         {
             ...
             int ntiles = numTilesInBox(box, /*tiling=*/true, bin_size);
             // printf("ntiles: %d\n", ntiles);
             // amrex::Print() << "A cell-centered Box " << box << "\n";
 
             // 用 DenseBins 做计数排序
             // amrex::Print() << plo[0] << " " << plo[1] << " " << plo[2] << " " << dxi[0] << " " << dxi[1] << 
             //     " " << dxi[2] << " " << domain << " " << bin_size << " " << box << "\n";
             // amrex::Print() << plo << " " << dxi << " " << domain << " " << bin_size << " " << box << "\n";
             // amrex::Print() << GetParticleBin{plo, dxi, domain, bin_size, box} << "\n";
             // amrex::Abort("Stop here");
             m_bins.build(np, ptile.getParticleTileData(), ntiles,
                        GetParticleBin{plo, dxi, domain, bin_size, box});
             // m_bins.build(np, ptile.getParticleTileData(), ntiles,
             ...
             // 取出排序结果
             auto* perm    = m_bins.permutationPtr();  // permutation array
             // auto* bins = m_bins.binsPtr();      // offset array
             int* offsets = reinterpret_cast<int*>(m_bins.offsetsPtr());      // offset array
             int numBins = m_bins.numBins();
             // long numItems = m_bins.numItems();
             // std::cout << "numBins: " << numBins << std::endl;
             // std::cout << "numItems: " << numItems << std::endl;
 
             // 调用 ParticleTile 里的 setIndexAndOffset (或你自己的 setIndexAndOffset)
             // ptile.setIndexAndOffset(perm, np, offsets, numBins);
             // CountSortTestCells(pti);
             
             // std::cout << "m_local_index shape: " << ptile.m_local_index.size() << std::endl;
             // std::cout << "m_bin_offsets shape: " << ptile.m_bin_offsets.size() << std::endl;
             // amrex::Abort("Stop here");
             // for(int i=0;i<numBins;i++){
             //     std::cout << "bins[" << i << "]: " << offsets[i] << std::endl;
             // }
             // for( auto i = 0; i < pti.m_local_index.size(); i++ ){
             //     std::cout << "m_local_index[" << i << "]: " << pti.m_local_index[i] << std::endl;
             //     std::cout << "m_bin_offsets[" << i << "]: " << pti.m_bin_offsets[i] << std::endl;
             // }
             // amrex::Abort("Stop here");
             ...
             std::vector<int> perm1(np);
             std::iota(perm1.begin(), perm1.end(), 0); // iota 函数将 perm 数组填充为 0, 1, 2, ..., np-1   
             // int* p_perm1=perm1.data();
             //todo
             // ptile.setIndexAndOffset(perm1.data(), np, offsets, numBins);
             int nps=static_cast<int>(np);
             ptile.buildPMAfromDenseBins(perm1.data(), nps, offsets, numBins,0.5);
             // ptile.buildPMAfromDenseBins_org(perm1.data(), nps, offsets, numBins);
             // printf("idd: %d, nps: %d, numBins: %ld\n", idd, nps, numBins);
             // CountSortTestPMA(pti);
 ...
 void
 WarpXParticleContainer::SortParticlesByCell_bak (amrex::IntVect bin_size=IntVect(AMREX_D_DECL(1, 1, 1)))
 {
     BL_PROFILE("WarpXParticleContainer::SortParticlesByBin()");
     // printf("WarpXParticleContainer::SortParticlesByCell() is called\n");
     ...
     for (int lev = 0; lev < numLevels(); ++lev)
     {
         ...
         for (WarpXParIter pti(*this, lev); pti.isValid(); ++pti)
         {
             ...
             // ptile.setIndexAndOffset(perm1.data(), np, offsets, numBins);
             int nps=static_cast<int>(np);
             ptile.buildPMAfromDenseBins(perm1.data(), nps, offsets, numBins,0);
             const long numcell = ptile.m_bin_offsets.size()-1;
             auto& pid_to_bin_map=ptile.m_pid_to_bin_map;
             std::vector<long> newbin(pid_to_bin_map.begin(),pid_to_bin_map.end());
             applyPendingMoves_nogpma(numcell, ptile, newbin,np);
 
             // ptile.buildPMAfromDenseBins_nogap(perm1.data(), nps, offsets, numBins);
             // ptile.buildPMAfromDenseBins_org(perm1.data(), nps, offsets, numBins);
             // printf("idd: %d, nps: %d, numBins: %ld\n", idd, nps, numBins);
             // CountSortTestPMA(pti);
 ...
 #endif
 void
 WarpXParticleContainer::CountSortTestPMA(WarpXParIter& pti)
 {
     ...
     Box tilebox1 = pti.tilebox();
     const int nx = tilebox1.length(0);
     const int ny = tilebox1.length(1);
     const int nz = tilebox1.length(2);
     const int num_bins = nx * ny * nz;
 
     // 临时数组用于计数排序
     std::vector<int> bin_counts(num_bins, 0);
     std::vector<int> offsets(num_bins + 1, 0);
     std::vector<int> sorted_indices(np);
 
     std::vector<int> particle_bins(np);
     ...
     for( int ip = 0; ip < np; ++ip ){
         ...
         const auto k = static_cast<int>(ymid);
         const double zmid = ((zp - xyzmin.z) + relative_time*vz)*dinv.z;
         const auto l = static_cast<int>(zmid);
 
         // 计算粒子属于哪个bin
         int bin_id = j-2 + (k-2)*nx + (l-2)*nx*ny;
         bin_counts[bin_id]++;
         particle_bins[ip] = bin_id;
         printf("ip: %d in: %d jn: %d kn: %d bin_id: %d\n", ip, j-2, k-2, l-2,bin_id);
     }
     // 计算前缀和
     offsets[0] = 0;
     for (int i = 0; i < num_bins; ++i){
         offsets[i+1] = offsets[i] + bin_counts[i];
     }
 
     // 将粒子索引根据bin的顺序排列
     std::vector<int> current_position = offsets;  // 用于记录每个bin填入到哪了
     for(int ip = 0; ip < np; ++ip){
         int bin = particle_bins[ip];
         int idx = current_position[bin]++;
         sorted_indices[idx] = ip;
     }
 
     // 到这里sorted_indices数组即为最终排序后的粒子索引
     // 测试打印（可选）
     auto& ptile = ParticlesAt(0, pti);
     ...
     for(int bin=1; bin<=num_bins; ++bin){
         int start = bin_offsets[bin-1];
         int end = bin_offsets[bin];
         int start1 = offsets[bin-1];
         int end1 = offsets[bin];
         if (start == end || start1 == end1) continue;
         // printf("bin: %d, start: %d, end: %d, start1: %d, end1: %d\n", bin, start, end, start1, end1);
         for(int i=0; i<bin_length[bin-1]; ++i){
             int local_i= start+i;
             int sort_i= start1+i;
             if(local_index[local_i]!=sorted_indices[sort_i]){
                 printf("local_index[%d]: %ld Sorted_indices[%d]: %d\n", 
                     local_i, local_index[local_i],sort_i, sorted_indices[sort_i]);
             }
         }
         // printf("\n");
         // for(int i=start1; i<end1; ++i){
         //     printf("sorted_indices[%d]: %d ", i, sorted_indices[i]);
         // }
         // printf("\n");
 ...
 void
 WarpXParticleContainer::CountSortTestCells(WarpXParIter& pti)
 {
     const auto GetPosition = GetParticlePosition<PIdx>(pti, 0);
     const amrex::Real clightsq = 1.0_rt/PhysConst::c/PhysConst::c;
     ...
     for(int bin=1; bin<=num_bins; ++bin){
         ...
         for(int i=start; i<end; ++i){
             if(local_index[i]!=sorted_indices[i]){
                 printf("local_index[%d]: %ld Sorted_indices[%d]: %d\n", i, local_index[i],i, sorted_indices[i]);
             }
         }
         // printf("\n");
         // for(int i=start1; i<end1; ++i){
  ...
  */
 void
 WarpXParticleContainer::DepositCurrent_rebuild (WarpXParIter& pti,
                                         ...
                                         amrex::Real const dt, amrex::Real const relative_time, PushType push_type)
 {
     ...
     // std::chrono::duration<double> els=st2-st1;
     // double time1 =els.count();
     // printf("lock add: %f\n",time1);
     // amrex::Abort("Stop");
         ...
         if (WarpX::current_deposition_algo == CurrentDepositionAlgo::Esirkepov) {
             ...
             if (push_type == PushType::Explicit) {
                 if        (WarpX::nox == 1){
                    ...
                    //     xyzmin, lo, hi,len, q, test,test0,testbin,test_sxm,ptile,tbox,
                    //     test_rhocells,test_sxwq,WarpX::n_rz_azimuthal_modes);
                    // doDepositionShapeN_sve_sort_real_sme<3>(
                    //         GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                    //         uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                    //         jx_fab,  jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                    //         xyzmin, lo, hi,len, q, test,test0,test1,ptile,tbox,
 ...
 #pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
        ...
        newbin.clear();
         ...
         for (WarpXParIter pti(*this, lev); pti.isValid(); ++pti)
         {
             ...
             tilebox.grow(ng_J);
             const Dim3 len = length(tilebox);
             const int numcells = (len.x+1)*(len.y-2)*(len.z);
            ...
            wqy.assign(np,0.0);
            wqz.assign(np,0.0);
            sxm.assign(np*(WarpX::nox+1),0.0);
            sym.assign(np*(WarpX::nox+1),0.0);
            szm.assign(np*(WarpX::nox+1),0.0);
            xrhocells.assign(8*numcells,0.0);
            yrhocells.assign(8*numcells,0.0);
            zrhocells.assign(8*numcells,0.0);
            newbin.assign(np,0.0);

             
             DepositCurrent_rebuild(pti, wp, uxp, uyp, uzp, ion_lev,
                            wqx,wqy,wqz,sxm,sym,szm,
                            xrhocells,yrhocells,zrhocells,newbin,
