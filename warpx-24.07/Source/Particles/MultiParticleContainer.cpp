...
RankSortStats
MultiParticleContainer::Evolve_rebuild (int lev,
                                const MultiFab& Ex, const MultiFab& Ey, const MultiFab& Ez,
                                const MultiFab& Bx, const MultiFab& By, const MultiFab& Bz,
                                MultiFab& jx, MultiFab& jy, MultiFab& jz,
                                MultiFab* cjx,  MultiFab* cjy, MultiFab* cjz,
...
double local_time = 0.0;
double kernal_time = 0.0;
double F = 5526.0;
double start_time=0.0;
double end_time=0.0;
// long total_particles=0;
long total_particles = TotalNumberOfParticles();
long total_cells = 0;
// printf("num particles: %ld\n",total_particles);


    // --- 初始化本 MultiPC 的总统计值 ---
    RankSortStats total_stats_for_multi_pc{0, 0, 0, 0.0};
    ...
    for (auto& pc : allcontainers) {
        for(WarpXParIter pti(*pc,lev);pti.isValid();++pti)
        {
            ...
            const int numcells = (len.x)*(len.y)*(len.z);
            total_cells += numcells;
        }
        start_time=MPI_Wtime();

        RankSortStats pc_stats = pc->Evolve_rebuild1(lev, Ex, Ey, Ez, Bx, By, Bz, jx, jy, jz, cjx, cjy, cjz,
                   rho, crho, cEx, cEy, cEz, cBx, cBy, cBz, t, dt, a_dt_type, skip_deposition, push_type);
        // pc->Evolve(lev, Ex, Ey, Ez, Bx, By, Bz, jx, jy, jz, cjx, cjy, cjz,
        //            rho, crho, cEx, cEy, cEz, cBx, cBy, cBz, t, dt, a_dt_type, skip_deposition, push_type);
        total_stats_for_multi_pc += pc_stats; // 使用重载的 += 运算符累加
        end_time=MPI_Wtime();
        local_time+=(end_time-start_time);
        // std::cout << "time1 " << pc_stats.local_time << std::endl;

    }
    kernal_time=total_stats_for_multi_pc.local_time;
    // std::cout << "time " << kernal_time << std::endl;
    double f_particle = 278;
    double f_cell = 32;

    double local_flops=f_particle*total_particles+f_cell*total_cells;
    double total_flops=0.0;
    double max_time=1.e12;
    double avg_time=1.e12;
    double total_mpi_time=0.0;
    MPI_Reduce(&local_flops,&total_flops,1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
    MPI_Reduce(&kernal_time,&max_time,1,MPI_DOUBLE,MPI_MAX,0,MPI_COMM_WORLD);
    MPI_Reduce(&kernal_time,&total_mpi_time,1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
    ...
    if(rank==0)
    {
        avg_time=total_mpi_time/rank_size;
        double ss_tflops=total_flops/(max_time*1e12);
        max_tflops=(max_tflops > ss_tflops) ? max_tflops : ss_tflops;
        printf("\nDeposition Kernal Sustained Performence: %f TFLOPS \n", ss_tflops);
        printf("\n\tPeak Performence: %f TFLOPS \n", max_tflops);
        printf("\n\tMax kernal time at step: %f s \n", max_time);
        printf("\n\tAvg kernal time at step: %f s \n", avg_time);
    }
    // printf("num particles: %ld\n",total_particles);
    return total_stats_for_multi_pc;
    
}

long MultiParticleContainer::TotalNumberOfParticles() const
{
    long total = 0;
    for (auto& pc : allcontainers) {
        total+=pc->TotalNumberOfParticles();
    }
    return total;
}
...
void
MultiParticleContainer::SortParticlesByCell (amrex::IntVect bin_size=IntVect(AMREX_D_DECL(1, 1, 1)))
{
    // printf("MultiParticleContainer::SortParticlesByCell\n");
    for (auto& pc : allcontainers) {
            pc->SortParticlesByCell(bin_size);
    }
}
