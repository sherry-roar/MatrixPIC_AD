...
RankSortStats
PhysicalParticleContainer::Evolve_rebuild1 (int lev,
                                   const MultiFab& Ex, const MultiFab& Ey, const MultiFab& Ez,
                                   const MultiFab& Bx, const MultiFab& By, const MultiFab& Bz,
                                   MultiFab& jx, MultiFab& jy, MultiFab& jz,
                                   MultiFab* cjx, MultiFab* cjy, MultiFab* cjz,
                                   ...
                                   PushType push_type)
{
    ...
    int step_rebuild_count = 0;
    long total_empty_slots_in_pc = 0;
    long total_capacity_in_pc = 0;
    double total_time_thread = 0.0;
    
#ifdef AMREX_USE_OMP
// #pragma omp parallel
#pragma omp parallel reduction(+:step_rebuild_count, total_empty_slots_in_pc, total_capacity_in_pc) \
 reduction(max:total_time_thread)
#endif
    {
#ifdef AMREX_USE_OMP
        const int thread_num = omp_get_thread_num();
        ...
        for (WarpXParIter pti(*this, lev); pti.isValid(); ++pti)
        {
            ...
            if (! do_not_push)
            {
                ...
                if (!skip_deposition)
                {
                    auto& ptile = ParticlesAt(lev, pti);  // <-- 正确获取粒子tile
                    ptile.m_was_rebuilt_this_step = false; // Reset before operations
                    ...
                    local_time_thread += elapsed_time;
                    
                    // --- Check the flag after the operation ---
                    if (ptile.m_was_rebuilt_this_step) {
                        step_rebuild_count++; // Reduction handles accumulation
                    }
                     // --- 累加空槽数和容量 ---
                    total_empty_slots_in_pc += ptile.m_num_empty_slots;
                    total_capacity_in_pc += ptile.m_capacity;
                } // end of "if electrostatic_solver_id == ElectrostaticSolverAlgo::None"
            } // end of "if do_not_push"
        ...
        total_time_thread=local_time_thread;
    }
    // Split particles at the end of the timestep.
    // When subcycling is ON, the splitting is done on the last call to
    // PhysicalParticleContainer::Evolve on the finest level, i.e., at the
    ...
    // printf("return RankSortStats{step_rebuild_count, total_empty_slots_in_pc,\n");
    return RankSortStats{step_rebuild_count, total_empty_slots_in_pc, total_capacity_in_pc, total_time_thread};
}
